...
function checkword() {
  ...
  const charValuesArray = Array.from(charElements).map(
    (charElement) => charElement.innerHTML
  );
  ...
  for (let i = 0; i < charValuesString.length; i++) {
    if (colors[i] === "GREEN") {
      ...
      document.getElementById(`${charValuesString[i]}`).style.backgroundColor =
        "#538d4e";
      ...
      if (
        !document
          .getElementById(`${charValuesString[i]}`)
          .classList.contains("used")
      ) {
...
function Enter() {
  ...
  const charValuesString = charValuesArray.join("");
  let charLower = charValuesString.toLowerCase();
  ...
  } else if (wordList.includes(charLower)) {
    checkword();
    roww += 1;
    charr = 1;
    if (roww > 6 && gameEnded == false) {
      exitgame(false); // Indicate loss
    }
  } else {
    message.textContent = "No Word in dictionery";
    message.style.color = "white";
    setTimeout(() => {
      message.textContent = "hello"; // Clear the message
      message.style.color = "black";
